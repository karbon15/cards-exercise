...
public class GameServiceControllerTests {
  ...
  public void shouldBeAbleToDeleteAGame() throws Exception {
    ...
    mockMvc.perform(delete("/v1/games/" + createResponse.getId()))
  ...
  public void shouldBeAbleToShuffle() throws Exception {
    ...
    for (CardBySuitAndValue cardBySuitAndValue : res.getCardsBySuitAndValue()) {
      Assertions.assertNotNull(res2.getCardsBySuitAndValue().stream()
          .filter(
              csv -> csv.getCardSuitAndValue().equals(cardBySuitAndValue.getCardSuitAndValue()) &&
                  csv.getCount().equals(cardBySuitAndValue.getCount()))
          .findAny());
    }
